unit SHL_WaveStream; // SpyroHackingLib is licensed under WTFPL
...
type
  ...
  public
    ...
    procedure WriteHeader(SampleRate: Integer = 0; Channels: Integer = 0;
      BitsPerSample: Integer = 0; DataSize: Integer = 0); overload;
    // overloaded to use booleans:
    procedure WriteHeader(SampleRate: Integer; IsStereo: Boolean; Is16Bit:
...
procedure TWaveStream.WriteHeader(SampleRate: Integer = 0; Channels: Integer = 0;
  BitsPerSample: Integer = 0; DataSize: Integer = 0);
var
  Header: TRiffHeader;
  OldPos: Integer;
begin
...
procedure TWaveStream.WriteHeader(SampleRate: Integer; IsStereo: Boolean;
  Is16Bit: Boolean = True);
begin
  // convert booleans to proper values:
  WriteHeader(SampleRate, 1 + Ord(IsStereo), 1 shl (3 + Ord(Is16Bit)));
end;

function TWaveStream.Write(const Buffer; Count: Integer): Integer;
begin
  if FStream <> nil then
...
function TWaveStream.Read(var Buffer; Count: Integer): Integer;
begin
  if FStream <> nil then
...
function TWaveStream.Seek(Offset: Integer; Origin: Word): Integer;
begin
  if FStream <> nil then
